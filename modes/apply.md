...
Interactive mode for when the candidate is filling in an application form in Chrome. Reads what is on screen, loads prior context for the offer, and generates personalised answers for each form question.
...
4. If there is a Section G → load the prior draft answers as a base
...
- **Notify the candidate**: "The role has changed from [X] to [Y]. Would you like me to re-evaluate, or shall I adapt the answers to the new title?"
